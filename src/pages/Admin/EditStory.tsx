...
export default function EditStory({ story }: { story: Story }) {
  ...
  const [year, setYear] = useState<string>(story.year);
  const [count, setCount] = useState<number>(story.count);
  const [type, setType] = useState(story.type);
  const [special, setSpecial] = useState<boolean>(story.special);
  ...
  async function handleSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(
        import.meta.env.VITE_DEV_API + "/stories/" + type + "/" + story.id,
        {
          method: "PUT",
          headers: {
